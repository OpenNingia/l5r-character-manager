...
def get_user_data_path(rel_path = '', org = "openningia", app = "l5rcm"):
    user_data = '.'
    if os.name == 'posix': # Linux is ok but Macosx ???
        user_data = os.path.join(os.environ['HOME'], '.local', 'share', app)
    elif os.name == 'nt':
        user_data = os.environ['APPDATA'].decode('latin-1')
        user_data = os.path.join(user_data, org, app)
